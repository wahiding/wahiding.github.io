...
- **The `gh` CLI here is authenticated as `wahiding`**, a different account from `wahidulalamnahid` (the name used across the actual site content/socials). Hosting was deliberately placed under `wahiding.github.io` per explicit user decision — don't try to "fix" this to match the personal name.
